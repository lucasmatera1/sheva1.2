...
const confrontationHistoryCache = new Map<string, DashboardPlayerMatchDetail[]>();
...
function ExpansionButton({ label, active, onClick, disabled = false }: { label: string; active: boolean; onClick: () => void; disabled?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function LoadingDot() {
  return <span className="inline-flex h-2.5 w-2.5 rounded-full bg-[#7a3f34] animate-pulse" aria-hidden="true" />;
}

function ArrowToggleButton({ direction, active, onClick, title }: { direction: "up" | "down"; active: boolean; onClick: () => void; title: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function MatchPill({ result, title }: { result: DashboardSequenceResult; title?: string }) {
  return (
    <span title={title} className={`inline-flex h-5 min-w-5 items-center justify-center rounded-full text-[10px] font-semibold ${getSequenceClass(result)}`}>
      {result}
    </span>
  );
}
...
function renderSequenceWithDividers(
  items: Array<{ key: string; result: DashboardSequenceResult; title?: string }>,
  ...
  return items.flatMap((item, index) => {
    ...
    if (variant === "button") {
      ...
      parts.push(<MatchPill key={item.key} result={item.result} title={item.title} />);
    }
...
function renderSequenceBlocks(
  ...
  if (!usesBlocks || items.length === 0) {
    return <div className="flex flex-wrap gap-1">{renderSequenceWithDividers(items, leagueType, variant)}</div>;
  }
        ...
        return (
          <Fragment key={`block-${blockIndex}`}>
            ...
            <div className="flex flex-col items-center">
              <span className="mb-0.5 text-[9px] font-bold tabular-nums text-ink/45">{pts}p</span>
              <div className="flex gap-0.5">
                {block.map((item) =>
                  variant === "button" ? (
                    <button key={item.key} type="button" title={item.title} className={`inline-flex min-w-9 justify-center rounded-full px-3 py-1 text-xs font-semibold ${getSequenceClass(item.result)}`}>{item.result}</button>
                  ) : (
...
function groupMatchesByOpponent(playerName: string, matches: DashboardPlayerMatchDetail[]) {
  return Array.from(
    matches.reduce((map, match) => {
      const key = `${playerName} x ${match.opponent}`;
      const current = map.get(key) ?? { opponent: match.opponent, matches: [] as DashboardPlayerMatchDetail[] };
...
function groupMatchesByDay(matches: DashboardPlayerMatchDetail[]) {
  return Array.from(
    ...
    .sort((left, right) => new Date(right.matches[0]?.playedAt ?? 0).getTime() - new Date(left.matches[0]?.playedAt ?? 0).getTime());
}

function ConfrontationGroup({ playerName, group, scopeKey, leagueType }: { playerName: string; group: DashboardMatchGroup; scopeKey: string; leagueType: DashboardSnapshotLeagueType }) {
  ...
  const [historyMatches, setHistoryMatches] = useState<DashboardPlayerMatchDetail[] | null>(cachedMatches);
  const [hasLoadedHistory, setHasLoadedHistory] = useState(Boolean(cachedMatches));
  const [isLoadingHistory, setIsLoadingHistory] = useState(false);
  const [historyLoadFailed, setHistoryLoadFailed] = useState(false);
  const latestMatch = group.matches[group.matches.length - 1] ?? group.matches[0];
  const expandedMatches = mergeMatchCollections(preloadMatches, historyMatches ?? []);
  ...
  const loadHistory = async (open: boolean) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/dashboard/confrontation-history?${query.toString()}`, { cache: "no-store" });
  ...
  return (
    <details className="group border-b border-ink/10 last:border-b-0" onToggle={(event) => loadHistory((event.currentTarget as HTMLDetailsElement).open)}>
      <summary className="cursor-pointer list-none px-4 py-4 [&::-webkit-details-marker]:hidden">
        <div className="grid gap-3 md:grid-cols-[1.15fr_0.55fr_1.6fr_0.42fr] md:items-center">
          <div>
            <p className="font-semibold text-ink">{playerName} x {group.opponent}</p>
            <p className="mt-1 text-xs text-ink/55">
              Ultimo: {formatDate(latestMatch?.playedAt ?? new Date().toISOString())}
            </p>
          </div>
          <p className="text-xs uppercase tracking-[0.14em] text-ink/58">{group.matches.length} jogo(s)</p>
          ...
          <div className="text-right text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-strong">
            {isLoadingHistory ? <span className="mr-2 normal-case tracking-normal text-ink/55">Carregando...</span> : null}
            {historyLoadFailed ? <span className="mr-2 normal-case tracking-normal text-[#7a3f34]">Falha ao carregar</span> : null}
        ...
        <div className="overflow-hidden rounded-[0.9rem] border border-ink/10 bg-white">
          ...
          <div>
            {matchesByDay.map((dayGroup) => {
              const wins = dayGroup.matches.filter((match) => match.result === "W").length;
              const dayRate = dayGroup.matches.length ? (wins / dayGroup.matches.length) * 100 : 0;

              return (
                <article key={`${scopeKey}-${group.opponent}-${dayGroup.dayLabel}`} className="grid gap-3 border-b border-ink/10 px-4 py-4 text-sm text-ink/78 last:border-b-0 md:grid-cols-[0.7fr_1.8fr_0.45fr] md:items-center">
                  <div>
                    <p className="text-sm font-semibold text-ink">{dayGroup.dayLabel}</p>
                    <p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-ink/48">{dayGroup.matches.length} jogo(s)</p>
                  </div>
                  <div>
                    {renderSequenceBlocks(
                    ...
                    <p className="mt-2 text-[10px] uppercase tracking-[0.12em] text-ink/48">Mais antigo | mais recente</p>
                  </div>
                  <div className="text-left md:text-right">
                    <span className="inline-flex rounded-full bg-[#20352e] px-3 py-1 text-sm font-semibold text-white">{formatPercent(dayRate, 0)}</span>
                  </div>
                </article>
              );
            })}
...
function PlayerRow({ player, leagueType }: { player: DashboardPlayer; leagueType: DashboardSnapshotLeagueType }) {
  const [showUpcoming, setShowUpcoming] = useState(false);
  const [showPast, setShowPast] = useState(false);
  const [showPreviousWindows, setShowPreviousWindows] = useState(false);
  const [lazyPreviousWindows, setLazyPreviousWindows] = useState<DashboardPlayerPreviousWindow[] | null>(null);
  const [isLoadingPreviousWindows, setIsLoadingPreviousWindows] = useState(false);
  const [previousWindowsLoadFailed, setPreviousWindowsLoadFailed] = useState(false);
  ...
  const canExpandUpcoming = player.upcomingWindowGames > 0 || upcomingFixtures.length > 0;
  const canExpandPast = player.totalGames > 0 || recentMatches.length > 0;
  const canExpandPreviousWindows = previousWindows.length > 0 || Boolean(player.hasPreviousWindows);
  ...
  const recentMatchesByOpponent = groupMatchesByOpponent(player.name, recentMatches).map((group) => ({
    ...group,
    expandedMatches: mergeMatchCollections(
      group.matches,
      historicalMatches.filter((match) => normalizeKey(match.opponent) === normalizeKey(group.opponent)),
    ),
  }));

  const loadPreviousWindows = async () => {
    if (previousWindows.length > 0 || !player.hasPreviousWindows || isLoadingPreviousWindows) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/dashboard/player-previous-windows?${query.toString()}`, { cache: "no-store" });
  ...
  return (
    <article className="rounded-[1.35rem] border border-ink/10 bg-white/72 p-4 shadow-sm">
      {showUpcoming ? (
        <div className="mb-4 rounded-[1.1rem] border border-ink/10 bg-white/80 px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.16em] text-brand-strong">Proximos jogos da janela</p>
              <p className="mt-1 text-sm text-ink/62">{upcomingFixtures.length} fixture(s) para {player.name}</p>
            </div>
            <ExpansionButton label="Recolher acima" active onClick={() => setShowUpcoming(false)} />
          </div>
          <div className="mt-4 overflow-hidden rounded-[0.95rem] border border-ink/10 bg-[#f8f4ea]">
            ...
            <div>
              {upcomingFixtures.map((fixture) => (
                <article key={fixture.id} className="grid gap-2 border-b border-ink/10 px-4 py-4 text-sm text-ink/78 last:border-b-0 md:grid-cols-[1.05fr_1.2fr_0.9fr_0.8fr] md:items-center md:gap-3">
                  <p className="text-[11px] uppercase tracking-[0.14em] text-brand-strong md:text-xs md:text-ink/70">{formatDate(fixture.playedAt)}</p>
                  ...
                  <p className="text-xs text-ink/55">{fixture.seasonId ?? "-"}</p>
                </article>
              ))}
            </div>
          </div>
      ...
      <div className="grid gap-2 xl:grid-cols-[minmax(0,0.92fr)_minmax(0,0.52fr)_minmax(0,2.18fr)_minmax(0,0.68fr)_minmax(0,0.38fr)] xl:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            {canExpandUpcoming ? (
              <ArrowToggleButton
                ...
                title={showUpcoming ? "Recolher proximos jogos" : "Expandir proximos jogos"}
              />
            ) : null}
            {canExpandPast ? (
              <ArrowToggleButton
                ...
                title={showPast ? "Recolher jogos encerrados" : "Expandir jogos encerrados"}
              />
            ) : null}
          </div>
          ...
          {canExpandPreviousWindows ? (
            <div className="mt-3">
              <div className="flex flex-wrap items-center gap-2">
                <ExpansionButton
                  label={showPreviousWindows ? "Ocultar J anteriores" : "Ver J anteriores"}
                ...
                {previousWindowsLoadFailed ? <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#7a3f34]">Falha ao carregar</span> : null}
              </div>
            </div>
          ) : null}
        </div>
        ...
        <div className="min-w-0 justify-self-end text-right text-[11px] leading-5 text-ink/60 xl:min-w-[138px]">
          <p>{player.latestPlayedAt ? `Ultimo: ${formatDate(player.latestPlayedAt)}` : "Ultimo: sem jogo"}</p>
          <p className="mt-1">{player.nextFixtureAt ? `Proximo: ${formatDate(player.nextFixtureAt)}` : "Proximo: sem fixture"}</p>
        </div>
      ...
      {showPast ? (
        <div className="mt-4 rounded-[1.1rem] border border-ink/10 bg-white/80 px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.16em] text-brand-strong">Jogos encerrados do dia</p>
              <p className="mt-1 text-sm text-ink/62">Lista por confronto com a sequencia W / D / L no recorte atual.</p>
            </div>
            <p className="text-xs uppercase tracking-[0.14em] text-ink/52">{recentMatches.length} jogo(s)</p>
          </div>
          ...
          <div className="mt-4 overflow-hidden rounded-[0.95rem] border border-ink/10 bg-[#f8f4ea]">
            ...
            <div>
              {recentMatchesByOpponent.map((group) => (
                <ConfrontationGroup key={`${player.id}-${group.opponent}`} playerName={player.name} group={group} scopeKey={`${player.id}-recent`} leagueType={leagueType} />
              ))}
            </div>
          </div>
        </div>
      ...
      {showPreviousWindows ? (
        <div className="mt-4 rounded-[1.1rem] border border-ink/10 bg-white/80 px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.16em] text-brand-strong">Sequencia dos J anteriores</p>
              <p className="mt-1 text-sm text-ink/62">Janelas anteriores do mesmo jogador, da mais recente para a mais antiga.</p>
            </div>
            <p className="text-xs uppercase tracking-[0.14em] text-ink/52">{isLoadingPreviousWindows ? "Carregando" : `${previousWindows.length} J`}</p>
          </div>
          ...
          {isLoadingPreviousWindows ? (
                  ...
                  <div className="rounded-[0.85rem] border border-ink/8 bg-[#f8f4ea] p-4">
                    <div className="flex flex-wrap gap-2">
                      {Array.from({ length: 8 }).map((_, index) => (
                        <span key={`skeleton-pill-left-${index}`} className="h-7 w-10 rounded-full bg-[#e7e1d2]" />
                      ))}
                    </div>
                  </div>
                  <div className="rounded-[0.85rem] border border-ink/8 bg-[#f8f4ea] p-4">
                    <div className="flex flex-wrap gap-2">
                      {Array.from({ length: 8 }).map((_, index) => (
                        <span key={`skeleton-pill-right-${index}`} className="h-7 w-10 rounded-full bg-[#efe9dc]" />
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-4 space-y-3">
              {previousWindows.map((window, index) => (
                <details key={`${player.id}-${window.key}`} open={index === 0} className="group rounded-[0.95rem] border border-ink/10 bg-[#f8f4ea] open:bg-white">
                  <summary className="cursor-pointer list-none px-4 py-3 [&::-webkit-details-marker]:hidden">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-sm font-semibold text-ink">{window.windowLabel} | {window.dayLabel}</p>
                        <p className="mt-1 text-xs text-ink/55">{window.totalGames} jogo(s) | {window.wins}W {window.draws}D {window.losses}L | {window.rangeLabel} | APX {formatPercent(getWindowApx(window))}</p>
                      </div>
                    ...
                    <div className="mt-3">
                      {renderSequenceBlocks(
                        window.matches.slice(0, MAX_SEQUENCE_ITEMS).map((match, matchIndex) => ({
                  ...
                  <div className="border-t border-ink/10 px-4 py-4">
                    <div className="overflow-hidden rounded-[0.95rem] border border-ink/10 bg-white/72">
                      ...
                      <div>
                        {groupMatchesByOpponent(player.name, window.matches).map((group) => (
                          <ConfrontationGroup key={`${window.key}-${group.opponent}`} playerName={player.name} group={group} scopeKey={window.key} leagueType={leagueType} />
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            <div className="mt-4 rounded-[0.95rem] border border-dashed border-[#7a3f34]/30 bg-[#fff5f2] px-4 py-5 text-sm text-[#7a3f34]">
              Nao foi possivel carregar os J anteriores agora. Tente abrir novamente em alguns segundos.
            </div>
          ) : (
            <div className="mt-4 rounded-[0.95rem] border border-dashed border-ink/15 bg-white/45 px-4 py-5 text-sm text-ink/60">
              Nenhum J anterior encontrado no recorte carregado para este jogador.
            </div>
          )}
        </div>
      ) : null}
...
function DashboardPlayerTable({ players, leagueType }: { players: DashboardPlayer[]; leagueType: DashboardSnapshotLeagueType }) {
  return (
    <Surface>
      <div className="hidden items-center gap-2 border-b border-ink/10 px-2 pb-3 text-[11px] uppercase tracking-[0.16em] text-brand-strong xl:grid xl:grid-cols-[minmax(0,0.92fr)_minmax(0,0.52fr)_minmax(0,2.18fr)_minmax(0,0.68fr)_minmax(0,0.38fr)]">
        <span>Player</span>
        <span>W / D / L do dia</span>
        <span>Sequencia</span>
        <span className="text-right">Ultimo / proximo</span>
        <span className="text-right">Apt.</span>
      </div>

      <div className="mt-4 space-y-3">
        {players.map((player) => (
          <PlayerRow key={player.id} player={player} leagueType={leagueType} />
        ))}
      </div>
    </Surface>
  );
