...
class UIBetPlacer:
    ...
    async def _reliable_click(
        ...
        *,
        ...
        Ordem de fallback:
        1. page.mouse.click(x, y) — CDP trusted event
        2. Se timeout:
    ...
    async def place_bet_by_signal(
        ...
        Inclui retry com backoff exponencial: se sr!=0 e sr!=14 (já tratado),
        re-tenta até max_retries vezes com delay 1s→2s→4s.
        ...
        for attempt in range(max_retries):
            try:
                ...
                # sr=14 já tratado acima. Para outros sr!=0, retenta.
                if not result.success and result.sr not in (-1, 0, 14) and attempt < max_retries - 1:
                    delay = 2 ** attempt  # 1s, 2s, 4s
                    logger.info(
                        "Retry {}/{}: sr={} — aguardando {}s (backoff exponencial)...",
                        attempt + 2, max_retries, result.sr, delay,
                    )
                    try:
                        await self.deselect_odds()
                        await self.close_betslip()
                ...
                break  # sucesso ou sem retry

            except Exception as e:
                result.error = str(e)
                logger.error("Erro no place_bet_by_signal (attempt {}): {}", attempt + 1, e)
                if attempt < max_retries - 1:
                    delay = 2 ** attempt
                    logger.info("Retry {}/{} após erro — {}s...", attempt + 2, max_retries, delay)
                    try:
                        await self.deselect_odds()
                        await self.close_betslip()
                    except Exception:
