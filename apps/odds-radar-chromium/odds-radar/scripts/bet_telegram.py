...
def _find_browser_hwnds():
    """Retorna lista de HWNDs das janelas do Camoufox (apenas processo camoufox/firefox, nunca Chrome)."""
    import ctypes
    ...
    _camoufox_pids: set = set()
    found = []

    def _is_camoufox_process(hwnd) -> bool:
        """Verifica se a janela pertence a um processo camoufox ou firefox (não chrome)."""
        pid = wintypes.DWORD()
        user32.GetWindowThreadProcessId(hwnd, ctypes.byref(pid))
        p = pid.value
        if p in _camoufox_pids:
            return True
        ...
        try:
            ...
            if kernel32.QueryFullProcessImageNameW(h, 0, buf, ctypes.byref(size)):
                exe = buf.value.lower()
                if "camoufox" in exe or ("firefox" in exe and "chrome" not in exe):
                    _camoufox_pids.add(p)
                    return True
        finally:
...
STAKE = 1.00
MAX_ODD_DROP = 0.15   # Queda máx aceitável da odd (sinal @1.83 aceita até 1.68)
...
async def main() -> None:
    ...
    safety = get_safety()
    print("=" * 60)
    print("  📡 TELEGRAM LISTENER + AUTO-BET")
    ...
    for gid in group_ids:
        try:
            ...
            print(f"⚠️ Grupo {gid} não encontrado: {e}")
    ...
    async def _handle_signal(event, is_edit=False):
        ...
        async with _bet_lock:
            try:
                ...
                # scrollIntoView no find_odds_by_player garante que o elemento fica visível
                # Mesmo fluxo comprovado no test_multi_bet.py: 3/3 aceitas
                ...
                if bet_result.success:
                    bet_count += 1
                    logger.info(
                        "APOSTA #{} ACEITA! sr=0 receipt={} odd={}",
                        bet_count, bet_result.bet_receipt, bet_result.odds,
                    )
                ...
                _bet_active.clear()
                logger.error("TIMEOUT GLOBAL (25s) — resetando para #/IP/FAV/")
                try:
                    for _ in range(3):
                        await page.keyboard.press("Escape")
                        await asyncio.sleep(0.2)
